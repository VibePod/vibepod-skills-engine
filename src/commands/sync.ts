...
export async function syncCommand(opts: SyncOptions): Promise<number> {
  ...
  if (ids.length === 0) {
    emit(
      { command: "sync", scope, restored: [], unchanged: [] },
      () => `Nothing to sync (${scope})`,
    );
    flush();
    return 0;
  }
  ...
  for (const id of ids) {
    ...
    try {
      const resolved = await fetchSource(entry.source.locator, {
        cacheDir: cacheRoot(),
      });
  ...
  if (failed.length > 0) {
    emit(
      { command: "sync", scope, restored, unchanged, failed },
      () =>
